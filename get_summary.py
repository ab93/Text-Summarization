...
for i in range(0,322):
	#computes the n value (the number of keywords) that TextRank must compute. This is calculated based on the below formula which was derived expermientally.
	n=int(math.ceil(min(0.1*length[i],7*math.log(length[i]))))
	try:
		#the reference summary is obtained using the vanilla TextRank algorithm proposed by Prof. Rada Mihalcea of Univ of Michigan
		reference=textSummarize.textSummarizeMain('spanishdata/2010-2013/2010-13c'+str(i)+'.txt',10)
		#the candidate summary is obtained using the new flavor of the TextRank algorithm v2.0
		#The new version uses keyword extraction as opposed to sentence extraction. Using these keywords, the sentences are then extracted using a calculated
		#"sentence score"
